...
export const createProject = createServerFn({ method: 'POST' })
  .validator(z.object({ doc: docSchema }))
  .handler(async ({ data }) => (await import('./projects')).insertProject(data.doc))

export const saveProject = createServerFn({ method: 'POST' })
  .validator(z.object({ id: z.string(), doc: docSchema }))
  .handler(async ({ data }) => {
...
export const renameProject = createServerFn({ method: 'POST' })
  .validator(z.object({ id: z.string(), name: z.string().min(1).max(120) }))
  .handler(async ({ data }) =>
...
export const saveThumbnail = createServerFn({ method: 'POST' })
  .validator(z.object({ id: z.string(), dataUrl: z.string().startsWith('data:image/') }))
  .handler(async ({ data }) =>
